...
export const normalizeRole = (value = "") => {
  ...
  if (["support", "supportagent", "support_agent"].includes(role))
...
export const buildPermissionCode = (moduleKey, action) => {
...
export const hasPermissionSet = (grantedPermissions = [], requested = "") => {
...
export const expandPermissionSet = (grantedPermissions = []) => {
  const permissions = Array.isArray(grantedPermissions)
    ? grantedPermissions
    : String(grantedPermissions || "")
        .split(",")
...
export const hasAnyPermissionSet = (
  grantedPermissions = [],
...
export const hasAllPermissionsSet = (
  grantedPermissions = [],
  requestedPermissions = [],
) => {
  const requests = Array.isArray(requestedPermissions)
...
export const getModulePermissionCode = (moduleKey, action = "view") =>
...
export const isActionSupported = (moduleKey = "", action = "view") =>
  getModuleActions(moduleKey).includes(normalizeToken(action));

export const normalizePermissionToken = normalizeToken;
